...
const Notifications = () => {
  ...
  const notifications = [...teacherNotifications, ...studentNotifications].sort((a, b) => b.timestamp - a.timestamp)
  ...
  const handleRecipientToggle = (recipient) => {
    setSelectedRecipients(prev =>
      prev.find(item => item.id === recipient.id)
  ...
  case 'specific-teachers':
    newNotification = sendToSpecificTeachers(title.trim(), message.trim(), teacherIds);
    break;
  case 'all-students':
  ...
  const getRecipientsList = () => {
    return notificationType.includes('teacher') ? guides : students
  }
        ...
        <div className="space-y-4">
          {notifications.length === 0 ? (
            ...
            notifications.map((notification) => (
              <div key={notification.id} className="border border-slate-200 rounded-xl p-6 hover:shadow-md transition-shadow duration-200">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-slate-500">{formatTimestamp(notification.timestamp)}</p>
                  </div>
                  <div className="ml-4">
