  ...
  useEffect(() => {
    ...
    if (userData && guides.length > 0) {
      try {
        ...
        const guide = guides.find(g => g.email === user.email || g.username === user.username)
        if (guide) {
          setCurrentGuide(guide)

          // Get projects assigned to this guide
          const projects = projectGroups.filter(group =>
            group.assignedGuide === guide.id && group.status === 'Accepted'
          )
          ...
          const guideStats = {
            ...
            unreadNotifications: teacherNotifications.filter(n => !n.read).length
          }
          setStats(guideStats)
    ...
    const recent = teacherNotifications
      .sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp))
      .slice(0, 3)
    setRecentNotifications(recent)
  }, [guides, projectGroups, tasks, students, teacherNotifications])
                    ...
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="flex items-center space-x-2">
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                          completionRate === 100 ? 'bg-green-100 text-green-700' :
